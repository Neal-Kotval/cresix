...
Implement the optional Cresix Cloud authority for target global account
handles, account-scoped workspace namespaces, installation
registration/revocation, bindings, bounded catalog projections, directory
pages, and relay route presence. It is a separate service and data store from
local C6.
...
The preview has no public account handle and guarantees workspace namespace
uniqueness only within its own database.
