...
function useRoute() {
  const normalize = (value: string) => value === "/settings/server" ? "/admin" : value.startsWith("/settings/peers") ? "/admin/access" : value;
  const [path, setPath] = useState(() => {
    const current = normalize(window.location.pathname);
    if (current !== window.location.pathname) history.replaceState({}, "", current);
    return current;
  });
  useEffect(() => {
...
export function App() {
  ...
  const visibleAdminSurface = adminSurface && !loading && session.serverAdministrator;
  ...
  return <div className="app-shell">
    <Topbar session={session} preview={preview} go={go} onMenu={() => setMobileNav((value) => !value)} adminSurface={visibleAdminSurface} />
    {notice && <Toast message={notice} onClose={() => setNotice(undefined)} />}
    <div className="shell-body">
      <Sidebar go={go} path={path} projects={loading ? [] : projects} workspace={loading ? undefined : session.workspaces[0]} open={mobileNav} onClose={() => setMobileNav(false)} adminSurface={visibleAdminSurface} serverAdministrator={!loading && session.serverAdministrator} />
      <main className="page">
        {loading && <div className="loading-bar" aria-label="Loading C6" />}
        {!loading && adminSurface ? !session.serverAdministrator ? <AdminDenied go={go} />
          : path === "/admin/access" ? <PeerTrust go={go} notify={setNotice} workspaceId={session.workspaces[0]?.id} currentUserId={session.user.id} preview={preview} />
          : <ServerSettings />
...
function NoWorkspace() {
  return <StatusPage icon={Users} title="No workspace access" body="An installation administrator must add this peer to a workspace before C6 Hub can show projects." />;
}

function AdminDenied({ go }: { go: (to: string) => void }) {
  return <StatusPage icon={LockKeyhole} title="C6 Admin is restricted" body="Workspace roles do not grant installation administration. Ask the installation administrator if you need access." action={<button className="button secondary" onClick={() => go("/")}>Return to C6 Hub</button>} />;
}

function Topbar({ session, preview, go, onMenu, adminSurface }: { session: Session; preview: boolean; go: (to: string) => void; onMenu: () => void; adminSurface: boolean }) {
  return <header className="topbar">
    <button className="mobile-menu" onClick={onMenu} aria-label="Open navigation"><Menu size={20} /></button>
    <div className="brand"><NavLink className="brand-home" to="/" go={go}><span className="brand-mark"><Cloud size={18} strokeWidth={2.4} /></span><span>C6</span></NavLink><nav className="surface-switch" aria-label="C6 surfaces"><NavLink to="/" go={go} className={!adminSurface ? "active" : ""}>Hub</NavLink>{session.serverAdministrator && <NavLink to="/admin" go={go} className={adminSurface ? "active" : ""}>Admin</NavLink>}</nav></div>
    {adminSurface ? <div className="surface-context"><ShieldCheck size={15} /><span>Installation administration</span></div> : <button className="global-search" onClick={() => document.getElementById("project-filter")?.focus()}><Search size={15} /><span>Find small software</span><kbd>⌘ K</kbd></button>}
...
function Sidebar({ go, path, projects, workspace, open, onClose, adminSurface, serverAdministrator }: { go: (to: string) => void; path: string; projects: ProjectSummary[]; workspace?: Workspace; open: boolean; onClose: () => void; adminSurface: boolean; serverAdministrator: boolean }) {
...
function Dashboard({ session, projects, go }: { session: Session; projects: ProjectSummary[]; go: (to: string) => void }) {
  ...
  return <div className="page-pad dashboard">
    ...
    <section className="server-strip"><div className="server-illustration"><Laptop size={25} /><span /><Server size={19} /></div><div><strong>C6 Hub is served by this installation</strong><p>Remote networking, DNS, and TLS remain operator-managed.</p></div>{session.serverAdministrator && <NavLink to="/admin" go={go}>Open Admin<ArrowRight size={14} /></NavLink>}</section>
...
function ProjectPage({ project, workspaceName, section, go, setProject, notify, preview, serverAdministrator }: { project: ProjectDetail; workspaceName?: string; section: string; go: (to: string) => void; setProject: (project: ProjectDetail) => void; notify: (message: string) => void; preview: boolean; serverAdministrator: boolean }) {
  ...
  return <>
    <header className="project-head"><div className="crumbs"><NavLink to="/" go={go}>{workspaceName ?? "Workspace"}</NavLink><span>/</span><strong>{project.name}</strong></div><div className="project-title-row"><div><div className="title-lockup"><span className="project-icon"><Braces size={24} /></span><h1>{project.name}</h1><span className="private-chip">Workspace</span></div><p>{project.description}</p></div><div className="actions">{serverAdministrator ? <NavLink className="button secondary" to="/admin/access" go={go}><ShieldCheck size={15} />Installation access</NavLink> : <button className="button secondary" disabled title="Workspace membership management is not implemented"><Users size={15} />Members planned</button>}<button className="button secondary" disabled title="Forking is not implemented"><GitFork size={15} />Fork planned</button></div></div>
...
function PeerTrust({ go, notify, workspaceId, currentUserId, preview }: { go: (to: string) => void; notify: (message: string) => void; workspaceId?: string; currentUserId: string; preview: boolean }) {
  ...
  return <div className="page-pad stack">{preview && <FixtureNotice label="Peer and device rows below are illustrative; invitations are the only connected control." />}<header className="page-title"><div><p className="kicker">C6 Admin · installation trust</p><h1>Access & invitations</h1><p>Global peer identities and invitation records for this C6 installation.</p></div><button className="button primary" onClick={() => setShowInvite(true)} disabled={Boolean(trustError)}><UserPlus size={15} />Invite peer</button></header>{trustError && <section className="security-callout" role="alert"><CircleAlert size={18} /><div><strong>Server administrator access required</strong><p>{trustError}</p></div></section>}<section className="trust-principle"><div className="trust-diagram"><span><Laptop size={19} /></span><i /><span><ShieldCheck size={19} /></span></div><div><strong>Administrator-issued invitations and cookie sessions</strong><p>These records cross workspace boundaries. Workspace ownership does not grant access.</p></div><NavLink to="/admin" go={go}>Review server state<ArrowRight size={14} /></NavLink></section>
...
function ServerSettings() { return <div className="page-pad stack"><FixtureNotice label="Counts below are illustrative; health and operator-boundary text reflect the current release." /><header className="page-title"><div><p className="kicker">C6 Admin · overview</p><h1>This C6 server</h1><p>Installation state, operating boundaries, and deferred capabilities.</p></div><span className="status success large"><i />API observed</span></header><section className="server-hero"><div className="server-machine"><Server size={34} /><span /></div><div><h2>This C6 process</h2><p>Local HTTP service · invitation-bound cookie sessions</p><div className="server-address"><Globe2 size={14} /><code>{location.origin}</code></div></div><dl><div><dt>Projects</dt><dd>Preview</dd></div><div><dt>Peers</dt><dd>Preview</dd></div><div><dt>Execution</dt><dd>Off</dd></div></dl></section><div className="metric-row"><article><HardDrive size={17} /><div><strong>Local metadata</strong><span>Configured · SQLite and Git</span></div></article><article><Activity size={17} /><div><strong>Safe simulation</strong><span>Configured · no project execution</span></div></article><article><Network size={17} /><div><strong>Operator-managed network</strong><span>Configured outside C6</span></div></article></div><section className="panel settings-panel"><PanelHead icon={Globe2} title="Connectivity" /><Setting title="Browser origin" body="Observed from this browser. C6 does not create DNS, TLS, or tunnels." control={<code>{location.origin}</code>} /><Setting title="C6-managed tunnel" body="Not implemented in this release." control={<span className="status neutral">Unavailable</span>} /><Setting title="HTTPS" body="Terminate TLS in an operator-managed reverse proxy before remote use." control={<span className="status neutral">Operator managed</span>} /></section><section className="panel settings-panel"><PanelHead icon={ShieldCheck} title="Administration" /><Setting title="Account recovery" body="No recovery flow exists yet. Keep an administrator session available." control={<span className="status neutral">Unavailable</span>} /><Setting title="Audit events" body="Security-sensitive local API operations are recorded." control={<span className="status success"><Check size={12} />Recorded</span>} /></section></div>; }
